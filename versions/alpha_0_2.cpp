...
struct ObjectLiteralExpr : Expr {
    ...
    std::string generateCode(CodeGen& gen) override {
        // For now, generate inline struct initialization
        std::string code = "{ ";
        bool first = true;
        for (auto& field : fields) {
            ...
            if (field.second.second) {
                code += field.second.second->generateCode(gen);
            } else {
                code += "0";
            }
        }
...
struct Declaration : Statement {
    ...
    void generateCode(CodeGen& gen) override {
        ...
        switch(type) {
            ...
            case OBJECT: cppType = "struct"; break;
            default: cppType = "auto"; break;
        }
        
        if (initExpr) {
            gen.writeLine(cppType + " " + name + " = " + initExpr->generateCode(gen) + ";");
        } else {
            ...
            gen.writeLine(cppType + " " + name + " = " + defaultVal + ";");
        }
    }
};
